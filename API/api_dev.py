...
model = pickle.load(open('../lgbm_home_risk_model.plk', 'rb'))
...
def home():
    ...
    for home risk project of OpenClassRooms DataScientist training.</p>"
...
def predict():
    ...
    # get inputs features from the data with index. Carreful you must pass data_index in int!!
    ...
    # create a dictionnary to janosify. We get only the probability to be positive!
    dict_result = {'ID Passenger':int(data_index), 'Survival Prediction Score': model_prediction[:,1][0]}
